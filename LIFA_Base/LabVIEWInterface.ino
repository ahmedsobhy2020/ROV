 ...
 **
 **
 **  Modified by:   Matt Lomanno - ISSI
 **  Modified on:   3/13/2013
 **  Have LIFA work over Ethernet instead of Serial 
 *********************************************************************************/
...
{  
  ...
  // gets the client and then determines how many bytes there are
  int bufferBytes = client.available();  
  if(bufferBytes >= COMMANDLENGTH) 
  {
    // New Command Ready, Process It  
    // Build Command From client Buffer
    for(int i=0; i<COMMANDLENGTH; i++)
    {
      currentCommand[i] = client.read();    
    }     
  ...
  if(command[0] == 0xFF && checksum_Test(command) == 0)
  {
    ...
    *********************************************************************************/
    ...
    case 0x07:    // Digital Read Port
      ...
      client.write( (retVal & 0xFF));
      client.write( (retVal >> 8));    
      break;
    ...
    *********************************************************************************/
    ...
    case 0x09:    // Analog Read Port
      ...
      //analog pin 0-5 are also D14-19
    case 0x2E:  // writeAnalogPin
      digitalWrite(command[2]+14, command[3]);
      client.write('0');
    break;  
    case 0x2F: //setAnalogPin
      pinMode(command[2]+14, command[3]);
        if(command[3]==1)
        {
          digitalWrite(command[2],LOW);
        }
    ...
    *********************************************************************************/
    ...
    case 0x10:    // I2C Read  
      ...
      {
        #if defined(ARDUINO) && ARDUINO >= 100
          client.write(Wire.read());
        #else
          client.write(Wire.receive());
        #endif
    ...
    *********************************************************************************/      
    ...
    case 0x1A:  // Servo Read Angle
      client.write(servos[command[2]].read());
      break;
    ...
    *********************************************************************************/        
     ...
     case 0x2C:  // Return Firmware Revision
         client.write(byte(FIRMWARE_MAJOR));  
         client.write(byte(FIRMWARE_MINOR));   
        break;  
    ...
    *********************************************************************************/
    ...
    // Checksum Failed, Flush client Buffer
    client.flush(); 
    client.write("error");
  }   
}
...
void analogReadPort()
{
  ...
  // Write Bytes To client Port
  client.print(output0);
  client.print(output1);
  client.print(output2);
  client.print(output3);
  client.print(output4);
  client.print(output5);
  client.print(output6);
  client.print(output7);
}
...
void spi_sendReceive(unsigned char command[])
{
  if(command[2] == 1)        //Check to see if this is the first of a series of SPI packets
  {
    ...
    for(int i=0; i<command[5]; i++)
    {
      ...
      // Send SPI Byte
      client.print(SPI.transfer(command[i+6]));     
      spiBytesSent++;  
  ...
  else
  {
    ...
    for(int i=0; i<command[3]; i++)
    {
      ...
      // Send SPI Byte
      client.write(SPI.transfer(command[i+4]));     
      spiBytesSent++;  
